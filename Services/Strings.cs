...
public static class Strings
{
    ...
    private static readonly Dictionary<AppLanguage, Dictionary<string, string>> Tables = new()
    {
        [AppLanguage.Zh] = new()
        {
            ...
            ["FirstRunTitle"] = "快速上手",
            ["FirstRunBody"] = "粘贴 Key 保存后，卡片开始显示额度。日常操作：右键卡片打开菜单（刷新 / 设置 / 隐藏到托盘），按住卡片拖动换位置。",
            ["KeyHint"] = "GLM Coding Plan 专用 Key，与 glm-plan-usage 插件同款",
            ["KeyOpenConsole"] = "打开控制台获取 →",
            ["ValidateTesting"] = "验证中…",
            ["ValidateSaveAnyway"] = "仍然保存",
            ["ValidateBadKey"] = "Key 验证失败（HTTP {0}）— 请检查 Key 与平台是否匹配",
            ["ValidateRejected"] = "Key 验证失败（{0}）— 请检查 Key 与平台是否匹配",
            ["ValidateNoData"] = "Key 可连通，但接口未返回额度数据 — 可能套餐不含 Coding Plan 额度或接口已变化。日志：{0}",
            ...
            ["Refreshing"] = "刷新中…",
            ["RefreshFailed"] = "刷新失败",
            ["RefreshFailedWhy"] = "刷新失败 · {0}",
            ["ErrNoQuotaData"] = "响应无额度数据",
            ["ErrTimeout"] = "超时",
            ["ErrNetwork"] = "网络错误",
            ["ErrTooltip"] = "刷新失败：{0}\n常见原因：Key 与平台不匹配、网络受限、套餐不含 Coding Plan 额度。\n详细日志：{1}",
            ["HiddenTitle"] = "已隐藏到托盘",
        ...
        [AppLanguage.En] = new()
        {
            ...
            ["FirstRunBody"] = "Paste your key and save — the rings light up. Right-click the card for the menu (refresh / settings / hide to tray); drag the card to move it.",
            ...
            ["ValidateSaveAnyway"] = "Save anyway",
            ["ValidateBadKey"] = "Key check failed (HTTP {0}) — verify the key and platform match",
            ["ValidateRejected"] = "Key check failed ({0}) — verify the key and platform match",
            ["ValidateNoData"] = "The key connects, but the API returned no quota data — the plan may not include Coding Plan usage, or the endpoint changed. Log: {0}",
            ...
            ["RefreshFailedWhy"] = "Failed · {0}",
            ["ErrNoQuotaData"] = "no quota data",
            ["ErrTimeout"] = "timeout",
            ["ErrNetwork"] = "network error",
            ["ErrTooltip"] = "Refresh failed: {0}\nCommon causes: key/platform mismatch, restricted network, or a plan without Coding Plan quota.\nFull log: {1}",
